...
def box_layer(inputs, anchors, num_classes):
    ...
    for lay in range(num_layers):
        box_xy, box_wh, box_confidence, box_class_probs = yolo_head(
            out[lay], anchors[anchor_mask[lay]], num_classes, input_shape)
    ...
    return boxes, scores[..., 1], scores[..., 0]
...
def yolo_text(num_classes, anchors, train=False):
    ...
    imgStrInput = Input(batch_shape=(None,), dtype=tf.string, name="input_str")
    imgInput = Lambda(load_base64_tensor)(imgStrInput)

    darknet = Model(imgStrInput, darknet_body(imgInput))
    ...
    if train:
        num_anchors = len(anchors)
        y_true = [Input(shape=(None, None, num_anchors // 3,
        ...
        lossList = [Lambda(get_loss, output_shape=(1,), name=lossName[
                           i], arguments={'index': i})(loss) for i in range(4)]
        textModel = Model([imgStrInput, *y_true], lossList)
        return textModel

    else:
        textModel = Model([imgStrInput], out)
        return textModel
